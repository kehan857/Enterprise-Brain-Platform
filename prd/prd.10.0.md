...
5.4.3 子菜单：数据映射配置 (Data Mapping Configuration)
...
5.4.4 子菜单：线下数据模板 (Offline Data Template)
    ○ 需求描述: 提供管理和使用标准数据模板以上传线下数据的界面。
    ○ 核心功能: 
        i. 模板列表展示:
            ■ 表格形式展示标准模板，包含以下列：模板名称、行业、业务领域、更新时间、状态、操作等。
            ■ 支持分页、排序功能。
        ii. 模板搜索功能:
            ■ 提供位于列表上方的搜索框，支持按模板名称进行快速模糊搜索。
            ■ 点击搜索框右侧下拉按钮，可展开高级搜索面板，支持多条件组合查询：
                □ 模板名称（文本框）
                □ 行业（下拉选择）
                □ 业务领域（下拉选择）
            ...
            ■ 高级搜索面板底部提供"重置"和"搜索"按钮。
        iii. 模板操作功能:
            ■ 提供"下载模板"按钮，点击后可下载选择的模板文件。
            ■ 提供"上传数据"按钮，点击后打开上传界面，引导用户选择模板和上传符合模板格式的数据文件。
            ■ 包含详细的数据上传说明和使用指南。
    ○ 页面原型: 包含搜索功能区域、操作按钮区（下载模板/上传数据）、模板列表表格（含指定列）、说明文字区域、分页控件。
    ○ 业务规则: 上传数据必须基于已有模板；系统会在后台校验上传数据的格式和有效性，并反馈校验结果；搜索条件可组合使用，支持精确和模糊匹配；搜索结果实时反馈，无结果时显示提示信息。
5.4.5 子菜单：数据质量 (Data Quality)
...
5.5.1 子菜单：文档管理 (Document Management)
    ...
    ○ 核心功能:
        ...
        ii. 文档管理列表 (右侧):
            ...
            ■ 分页: 列表支持分页功能。
        iii. 文档搜索功能:
            ■ 提供位于文档列表上方的搜索框，支持按文档名称进行快速模糊搜索。
            ■ 点击搜索框右侧下拉按钮，可展开高级搜索面板，支持多条件组合查询：
                □ 文档名称（文本框）
                □ 分类（下拉选择，与目录树联动）
                □ 文件大小（范围选择）
                ...
                □ 更新时间（日期范围选择）
            ■ 高级搜索面板底部提供"重置"和"搜索"按钮。
            ■ 搜索结果会显示在右侧列表区域，替代当前目录下的文档列表，同时显示搜索状态提示。
            ■ 可通过清空搜索条件或点击目录树节点退出搜索结果视图。
        iv. 上传文档: 点击"上传文档"按钮，弹出文件选择框，用户选择本地文件上传。上传时需允许用户选择目标目录，并必须指定要关联的一个或多个【文档知识库】实例。
    ○ 页面原型: 包含左侧目录树、右侧文档列表区域（含搜索功能、操作按钮、文档表格、分页控件）。
...
5.5.2 子菜单：FAQ管理 (FAQ Management)
    ...
    ○ 页面原型: 包含左侧FAQ分类树、右侧FAQ列表区域（含统一样式的搜索功能、操作按钮、FAQ表格、分页控件）。
    ○ 业务规则: FAQ必须属于某个分类；标签可用于增强检索；状态控制FAQ是否在问答中可用；创建/编辑时必须关联知识库；导入功能需定义好文件格式规范；搜索条件可组合使用，支持跨分类搜索；搜索结果实时反馈，无结果时显示提示信息；支持在搜索结果中高亮匹配关键词。
...
5.6.1 子菜单：分析报告 (Analysis Reports)
    ...
    ○ 核心功能: 
        i. 报告列表展示:
            ■ 以表格形式展示用户权限内的分析报告
            ■ 表格包含以下列：标题、创建人、业务域、创建时间、状态、操作（预览、下载）等
            ■ 支持分页、排序功能
        ii. 报告搜索功能:
        ...
        iii. 快捷筛选标签:
            ...
            ■ 点击标签可快速筛选结果，支持组合筛选
        iv. 提供清晰的文字说明和"前往报告中心查看分析报告"的链接按钮，点击后跳转至【5.7 报告中心】并自动筛选出由"智能分析中心"配置的Agent所生成的报告。
...
【优化】5.11.1 需求描述:
    为用户提供系统使用指南、常见问题解答、视频教程等支持资源的中心化访问点。帮助用户快速掌握平台功能，解决使用过程中遇到的问题，提升用户体验。
【优化】5.11.2 核心功能:
    ○ 使用指南: 提供平台各功能模块的详细使用说明，包含步骤指导和截图展示。
    ○ 常见问题(FAQ): 整理用户常见问题及解答，支持分类浏览和关键词搜索。
    ○ 视频教程: 提供关键功能的操作视频教程，直观展示操作流程。
    ○ 联系支持: 提供用户获取进一步支持的渠道信息。
【优化】5.11.3 页面原型:
    ○ 帮助中心主页: 顶部搜索框，下方分区展示主要内容类别卡片（使用指南、FAQ、视频教程、联系支持）。
    ○ 使用指南页: 左侧功能目录树，右侧内容区域（含文字说明、步骤引导、截图示例）。
    ○ FAQ页: 分类标签筛选、搜索框、可折叠问答列表。
    ○ 视频教程页: 视频缩略图卡片列表，点击可播放教程视频。
【优化】5.11.4 业务规则:
    ○ 内容需定期更新，确保与最新功能同步。
    ○ 帮助资源应保持简洁明了，避免过于技术化的描述。
    ○ 所有资源按模块和功能分类组织，便于用户查找。

5.13 智能助手 (Intelligent Assistant)
...
6.1.4 各模块默认搜索字段配置
    ...
    6.1.4.8 报表中心 - 报告列表
        • 简单搜索：报告名称、创建人、标签
        • 高级搜索：报告名称、报告类型、创建人、生成时间、业务板块、关联对象、来源模块、状态、标签
